...
export function PrimaryCta({ placement = "cta" }: { placement?: string }) { return <BookingLink className="button button-gold" placement={placement}>Book your ride <span aria-hidden="true">↗</span></BookingLink>; }
...
export function Breadcrumbs({ items }: { items: { label: string; href?: string }[] }) { return <nav className="breadcrumbs" aria-label="Breadcrumb">{items.map((item, index) => <span key={item.label}>{index > 0 && <span aria-hidden="true">/</span>}{item.href ? <Link href={item.href}>{item.label}</Link> : <span aria-current="page">{item.label}</span>}</span>)}</nav>; }

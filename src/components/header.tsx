...
export function Header({ locale = "en" }: { locale?: Locale }) {
  ...
  return <header className="site-header">
    <div className="utility-bar"><div className="container utility-inner"><span>{copy.utility}</span><TrackedContactLink channel="phone" href={phoneHref(phone)} placement="utility">{copy.call} {phone}</TrackedContactLink></div></div>
    <div className="container nav-shell">
      <Link aria-label="Revival Transportation Group" className="brand notranslate" data-no-translate href={localized("/")} onClick={() => setOpen(false)} translate="no"><span aria-hidden="true" className="brand-mark">R</span><span aria-hidden="true"><strong>Revival</strong><small>Transportation Group</small></span></Link>
